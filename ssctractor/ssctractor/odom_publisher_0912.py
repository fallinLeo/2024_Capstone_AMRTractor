...
class Odom_class(Node):
    def __init__(self):
        ...
        # self.frame_id_of_map = 'map'
        self.frame_id_of_odometry = 'odom'
        self.child_frame_id_of_odometry = 'base_footprint'
        ...
        if self.use_imu:
            queue_size = 10
            slop = 0.5  # 이 값은 메시지 동기화의 대략적인 시간 차이를 설정합니다. 필요에 따라 조정해야 할 수 있습니다.

            self.joint_state_sub = self.create_subscription(JointState,'joint_states',self.joint_state2_callback,10)
    ...
    def update_joint_state(self, joint_state: JointState,duration: Duration):
        ...
        self.diff_joint_positions[1] = enc_R      #두개의 단위 라디안 (변화량)
    ...
    def update_imu(self, imu_msg):
        ...
        self.robot_vel[2] = imu_msg.angular_velocity.z
        print(f"updated_imu_angle : {self.imu_angle}")
    ...
    def joint_state2_callback(self, msg):
        self.joint_state_data = msg

        if self.imu_data is not None:
            self.process_joint_and_imu(self.joint_state_data, self.imu_data)
    ...
    def process_joint_and_imu(self, joint_state_msg, imu_msg):
        # 현재 메시지의 시간을 저장
    ...
    def calculate_odometry(self, duration: Duration):
        wheel_l = self.diff_joint_positions[0] #각 바퀴 라디안단위 변화량
        ...
        delta_theta = 0.0
        theta = 0.0
        v=0.0 #translational velocity [m/s]
        w=0.0 #rotational velocity [rad/s]
        ...
        if self.use_imu:
            ...
            delta_theta = np.float64(theta) - np.float64(self.last_theta)
            # print(f"kinematic theta = {self.wheels_radius * (wheel_r - wheel_l) / self.wheels_separation}")
            print(f"delta_theta = {delta_theta} 단위 : rad") #joint_imu_pub코드확인
        else:
        ...
        w = delta_theta / step_time #rad/s

        self.robot_vel[0] = v
        self.robot_vel[1] = 0.0
        # self.robot_vel[2] = w

        self.last_theta = self.imu_angle
