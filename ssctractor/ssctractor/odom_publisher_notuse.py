...
class Odom_class(Node):
    ...
    def update_imu(self, imu_msg):
        ...
        self.get_logger().info(f"upt_imu_angle : {self.robot_vel[2]}")
    ...
    def calculate_odometry(self, duration: Duration):
        ...
        if self.use_imu:
            ...
            self.get_logger().info(f"delta_theta = {delta_theta} 단위 : rad") #joint_imu_pub코드확인
        else:
        ...
        self.get_logger().info(f"robot_pose[2] = {self.robot_pose[2]}")
