        ...
        if ids is not None:
            for i, corner in enumerate(corners):
                ...
                if ret:
                    ...
                    y_axis_rotation_quat = transforms3d.quaternions.axangle2quat([0, 1, 0], np.pi/2)
                    transformation_matrix = np.array([[0, 0, 1],  # Z -> X
                                                  [0, 1, 0],  # Y -> Y (변하지 않음)
                                                  [1, 0, 0]])  # X -> Z
                    # final_quat = transforms3d.quaternions.qmult(y_axis_rotation_quat, quat)

                    tvec_adjusted = np.dot(transformation_matrix, tvec)

                    # tvec_adjusted = [tvec[2], tvec[1], tvec[0]]
                    ...
                    pose_msg.pose.position.x = tvec_adjusted[0][0] / 1000.0
                    pose_msg.pose.position.y = tvec_adjusted[1][0] / 1000.0
                    pose_msg.pose.position.z = tvec_adjusted[2][0] / 1000.0
                    ...
                    t.transform.translation.x = tvec_adjusted[0][0] / 1000.0
                    t.transform.translation.y = -tvec_adjusted[1][0] / 1000.0
                    t.transform.translation.z = tvec_adjusted[2][0] / 1000.0
