...
class PathFollower(Node):
    ...
    def timer_callback(self):
        ...
        if self.path_created:
            if self.kim_flag ==False:
                self.follow_path(self.waypoints)
            if self.kim_flag==True:
                # self.kim_trajectory(self.goal_x,self.goal_y)
                pass
            self.get_logger().info(f'kim_flag : {self.kim_flag}')
 

    def callback(self, goal_pose):
        # if self.doc_flag ==False : return

        offset_distance = 0.3  # 50cm = 0.5m
        goal_x_offset = offset_distance * math.cos(self.yaw)
        goal_y_offset = offset_distance * math.sin(self.yaw)
        ...
        if not self.path_created:
            self.waypoints = self.generate_path(self.current_pose.pose.position.x, self.current_pose.pose.position.y, goal_x_offset, goal_y_offset)
    ...
    def follow_path(self, waypoints):
        ...
        #path의 맨 끝 도달 했을때 조건
        if self.current_index >= len(waypoints) or goal_to_current < 0.5:
            self.kim_flag =True
            return
        ...
        self.get_logger().info(f'error_heading: {error_heading}')

        delta_time = 0.01
        self.integral_error += error_heading * delta_time

        i_error = self.integral_error
        d_error = error_heading - self.prev_error
        angular_output = self.Kp * error_heading + self.Kd * d_error + self.Ki * i_error
    ...
    def kim_trajectory(self,goal_x,goal_y):
            cmd = Twist()
            linear_value = math.sqrt(goal_x **2 +goal_y **2)
            ...
            cmd.linear.x = -((linear_value-input_min)*(output_max - output_min) / (input_max - input_min) + output_min)
            cmd.angular.z = (math.atan2(goal_x,goal_y))
