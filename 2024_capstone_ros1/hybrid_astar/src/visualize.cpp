...
void Visualize::publishNode3DPose(Node3D& node) {
  ...
  pose.header.seq = 0;
  pose.pose.position.x = node.getX() * Constants::cellSize + map_x;
  pose.pose.position.y = node.getY() * Constants::cellSize + map_y;
...
void Visualize::publishNode3DPoses(Node3D& node) {
  ...
  pose.position.x = node.getX() * Constants::cellSize + map_x;
  pose.position.y = node.getY() * Constants::cellSize + map_y;
...
void Visualize::publishNode2DPose(Node2D& node) {
  ...
  pose.header.stamp = ros::Time::now();
  pose.header.seq = 0;
  pose.pose.position.x = (node.getX() + 0.5) * Constants::cellSize + map_x;
  pose.pose.position.y = (node.getY() + 0.5) * Constants::cellSize + map_y;
...
void Visualize::publishNode2DPoses(Node2D& node) {
  if (node.isDiscovered()) {
    geometry_msgs::Pose pose;
    pose.position.x = (node.getX() + 0.5) * Constants::cellSize + map_x;
    pose.position.y = (node.getY() + 0.5) * Constants::cellSize + map_y;
...
void Visualize::publishNode3DCosts(Node3D* nodes, int width, int height, int depth) {
  ...
  for (int i = 0; i < width * height; ++i) {
    ...
    if (values[i] != 1000) {
      ...
      // center in cell +0.5
      costCube.pose.position.x = (i % width + 0.5) * Constants::cellSize + map_x;
      costCube.pose.position.y = ((i / width) % height + 0.5) * Constants::cellSize + map_y;
