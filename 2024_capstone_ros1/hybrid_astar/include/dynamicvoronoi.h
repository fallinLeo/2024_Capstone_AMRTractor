...
class DynamicVoronoi {
 ...
 public:
  ...
  void initializeMap(int _sizeX, int _sizeY, bool** _gridMap, float _originX, float _originY);
  ...
  float getDistanceEdge( int x, int y ) const;
 ...
 public:
  ...
  const Node3D* voronoiAstar(int sx, int sy, int gx, int gy, CollisionDetection& configurationSpace);
  // const Node3D* makesubGoal(int sx, int sy, int gx, int gy);
  bool makesubGoal(int sx, int sy, int gx, int gy, CollisionDetection& configurationSpace);

  std::vector<Node3D>& getsubgoal(){return subGoalnodes;}

  visualization_msgs::MarkerArray& getsubGoalMarkerArray() {return subGoalMarkerArray;}
  nav_msgs::OccupancyGrid& getvoroCostmap() {return voroCostmap;}
  

  // queuess

  BucketPrioQueue open;
  BucketPrioQueue closest;
  std::queue<INTPOINT> pruneQueue;

  std::vector<INTPOINT> removeList;
  std::vector<INTPOINT> addList;
  std::vector<INTPOINT> lastObstacles;
  std::vector<INTPOINT> voroList;
  ...
  // visualization
  nav_msgs::OccupancyGrid voroCostmap;
  visualization_msgs::MarkerArray subGoalMarkerArray;
  Node3D* nodes3D;
  std::vector<Node3D> subGoalnodes;
