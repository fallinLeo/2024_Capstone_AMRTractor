...
inline void collisionLookup(Constants::config* lookup, float length, float width, float bbSize) {
  bool DEBUG = false;
  std::cout << "I am building the collision lookup table..."<< length << " " << width << " cell size" << Constants::cellSize << std::endl;
  // cell size
  const float cSize = Constants::cellSize;
  // bounding box size length/width
  const int size = bbSize;
  ...
  for (int q = 0; q < positions; ++q) {
    ...
    p[0].x = c.x - length / 2 / cSize;
    p[0].y = c.y - width / 2 / cSize;

    p[1].x = c.x - length / 2 / cSize;
    p[1].y = c.y + width / 2 / cSize;

    p[2].x = c.x + length / 2 / cSize;
    p[2].y = c.y + width / 2 / cSize;

    p[3].x = c.x + length / 2 / cSize;
    p[3].y = c.y - width / 2 / cSize;
    ...
    for (int o = 0; o < Constants::headings; ++o) {
      ...
      Constants::relPos zeroPose = {0,0};
      for (int i = 0; i < size; ++i) {
        for (int j = 0; j < size; ++j) {
          if (cSpace[i * size + j]) {
            // compute the relative position of the car cells
            lookup[q * Constants::headings + o].pos.push_back(zeroPose);
